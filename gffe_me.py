...
def gffe_me(world_pos, world_pos_1, mv, color, depth, stencil, vp_matrix, vp_matrix_next, program):
    ...
    glDeleteTextures(9, [world_pos_tex, world_pos_1_tex, mv_tex, depth_tex, color_tex, stencil_tex, warp_color_tex, warp_mv_tex, warp_depth_tex])
    glFinish()
...
def gffe_me_init():
    # 初始化opengl和创建着色器
    create_window(1280, 720, "gffe_me")
    with open("shaders/gffe_me.comp", "r") as f:
...
def gffe_me_main(label_index, label_path, seq_path, save_path, scene_name, program, debug=False):
    """
    ...
    warp_color, warp_mv, warp_depth = gffe_me(world_pos, world_pos_1, mv, depth, color, stencil, vp_matrix, vp_matrix_next, program)
